    ...
    def build_hist_keyboard(
        ...
        halaman_data = hasil.iloc[start:end]

        # -----------------------------------------------------
        # BUTTON CUSTOMER
        # -----------------------------------------------------

        for index, row in halaman_data.iterrows():
    ...
    async def hist_detail_handler(
        ...
        try:
            ...
            callback_owner_id = int(parts[1])
            index = int(parts[2])

        except Exception:

            await query.answer(
                "❌ Data tombol tidak valid.",
                show_alert=True
            )
            return

        # -----------------------------------------------------
        # USER LAIN TIDAK BOLEH MENEKAN
        # -----------------------------------------------------

        if int(user.id) != callback_owner_id:

            await query.answer(
                "⛔️ Tombol ini bukan untuk Anda.",
                show_alert=True
            )

            print(
                f"[HIST BLOCKED] User={user.id} "
                f"mencoba menekan tombol milik User={callback_owner_id}"
            )
        ...
        session_owner_id = context.user_data.get(
            "hist_owner_id"
        )

        if session_owner_id != int(user.id):

            await query.answer(
                "⛔️ Session HIST Anda sudah tidak tersedia.",
                show_alert=True
            )
        ...
        try:
            ...
            print(
                "[HIST DETAIL] "
                f"Foto berhasil dikirim. CUST ID={cust_id}"
            )
    ...
    async def hist_back_handler(
        ...
        if int(user.id) != callback_owner_id:
            ...
            print(
                f"[HIST BACK BLOCKED] User={user.id} "
                f"mencoba menekan tombol milik User={callback_owner_id}"
            )
    ...
    async def hist_page_handler(
        ...
        if int(user.id) != callback_owner_id:
            ...
            print(
                f"[HIST PAGE BLOCKED] User={user.id} "
                f"mencoba menekan pagination milik User={callback_owner_id}"
            )
        ...
        if not keyword:
            ...
            await query.answer(
                "❌ Session pencarian sudah tidak tersedia.",
                show_alert=True
            )
